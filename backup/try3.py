...
Environment
  - OPENAI_API_KEY   : API key for any OPENAI compatible endpoints
  - OPENAI_API_BASE  : base URL for the OpenAI API (e.g. https://api.poe.com/v1)
  - LLM_MODEL     : LLM model name (e.g. "Claude-Sonnet-4", "Llama-3.1-405B")
  - DB_PATH       : path to a local SQLite database file (e.g. ./sample.db)

Run
  $ pip install langgraph openai pandas
  $ export OPENAI_API_KEY=...; export LLM_MODEL=Claude-Sonnet-4; export DB_PATH=./sample.db
...
def llm_action(messages: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    print(f"LLM response: {content_stripped[:200]}...")  # Debug output
...
def print_step_at_start(title):
    print(f"\n=== {title} ===")

def print_step_at_end(result):
    print(f"\n{result}")

def node_call_model(state: AgentState) -> AgentState:
    print_step_at_start("I'm thinking...")
    ...
    print_step_at_end(new_state)

    return new_state

def node_execute_sql(state: AgentState) -> AgentState:
    print_step_at_start("Executing SQL...")
...
def node_execute_python(state: AgentState) -> AgentState:
    print_step_at_start("Executing Python...")
